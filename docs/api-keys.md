...
litsurvey uses four public services. None requires payment. Only one offers
a key, and it is optional.

| Service | Needs | Without it |
|---|---|---|
| OpenAlex | nothing; an email address gets the faster "polite pool" | works, slightly slower |
| Semantic Scholar | nothing; a free API key gives a dedicated 1 request/second | works on a shared pool; frequent rate-limit retries, `related` most affected |
| arXiv | nothing | n/a |
| Unpaywall | an email address | works with a placeholder address |
...
Check with `litsurvey doctor`; the second line should say
`S2 API key : set (s2k-…)`.
